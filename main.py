...
def agendar():
    ...
    if estudiante == "" or tutor == "" or materia == "":
...
def ver_tutorias():
    lista_tutorias.delete(0, tk.END)
    datos.cursor.execute("SELECT estudiante, tutor, materia FROM tutorias")
    for t in datos.cursor.fetchall():
        lista_tutorias.insert(tk.END, f"{t[0]} - {t[1]} ({t[2]})")
...
entry_tutor = tk.Entry(ventana); entry_tutor.pack()
entry_materia = tk.Entry(ventana); entry_materia.pack()
...
entry_estudiante = tk.Entry(ventana); entry_estudiante.pack()
...
lista_tutores = tk.Listbox(ventana); lista_tutores.pack(pady=5)
tk.Button(ventana,text="Mostrar Tutores",command=ver_tutores).pack()

# BUSCAR POR MATERIA
tk.Label(ventana,text="Buscar tutor por materia").pack()
entry_buscar = tk.Entry(ventana); entry_buscar.pack()
tk.Button(ventana,text="Buscar",command=buscar_materia).pack()

# AGENDAR TUTORIA
tk.Label(ventana,text="Agendar Tutoría").pack()
entry_estudiante_tutoria = tk.Entry(ventana); entry_estudiante_tutoria.pack()
entry_tutor_tutoria = tk.Entry(ventana); entry_tutor_tutoria.pack()
entry_materia_tutoria = tk.Entry(ventana); entry_materia_tutoria.pack()
tk.Button(ventana,text="Agendar",command=agendar).pack(pady=10)

# VER TUTORIAS
tk.Label(ventana,text="Tutorías agendadas").pack()
lista_tutorias = tk.Listbox(ventana); lista_tutorias.pack(pady=5)
tk.Button(ventana,text="Mostrar Tutorías",command=ver_tutorias).pack()
